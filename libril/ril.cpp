...
static void listenCallback (int fd, short flags, void *param) {
  ...
  //  RecordStram *p_rs is made as global variable.
    ...
    if (ret <= 0) {
        LOGE("Client is not passing proper subscription values");
    ...
    if (strncmp(p_record, SUB1, SUB_DATA_LENGTH) == 0) {
        p_rsInfo->client_id = 0;
        LOGI("Client ID :: %d", p_rsInfo->client_id);
    } else if (strncmp(p_record, SUB2, SUB_DATA_LENGTH) == 0) {
        p_rsInfo->client_id = 1;
        LOGI("Client ID :: %d", p_rsInfo->client_id);
    } else {
        LOGE("Should not come here !!! buffer red :: %s", p_record);
    }
...
static void debugCallback (int fd, short flags, void *param) {
    ...
    if (ret <= 0) {
        close(acceptFD);
        LOGE ("error Client is not sending proper subscription information");
        return;
    }
    client_id = 0;
    if (strncmp(p_record, SUB1, SUB_DATA_LENGTH) == 0) {
        client_id = 0;
        LOGI("Debug Client ID :: %d", client_id);
    } else if (strncmp(p_record, SUB2, SUB_DATA_LENGTH) == 0) {
        client_id = 1;
        LOGI("Debug Client ID :: %d", client_id);
    } else {
        LOGE("Should not come here !!! debug socket buffer red :: %s", p_record);
    }
    s_fdCommand[client_id]= acceptFD;
